...
4. **`site-nav` のリンク一覧は全ページに複製されている**。ページを追加・改名・並べ替えたら、`index.html` を含む**全 HTML の `site-nav` を漏れなく更新する**（下記）

## 共通マークアップ（ナビ・ヘッダー）の重複について

本プロジェクトは「バニラ HTML・ビルドツールなし」方針のため、テンプレートやサーバーインクルードを使わない。
その帰結として、上部の `site-header` と `site-nav`（ツール間メニュー）の HTML は**全ページに物理的にコピーされている**。これは**意図的な割り切り**であり、共通化のための仕組みは導入しない。

- 注意点: ナビの項目（リンク先・表示順・ラベル）を変えるときは、**全 HTML を 1 つずつ手で直す**。1 ファイルでも直し忘れると、そのページだけメニューがズレる（新ページへのリンクが無い等）。
- 変更後の確認: 各ページの `site-nav` のリンク集合が一致しているか、`aria-current="page"` が自ページを指しているかを見る。
- 補足: localStorage キーのようなロジック層は `storage-keys.js` に一元化しているが（[specification.md](specification.md) 第5.1）、**マークアップ層はあえて一元化しない**。JS 注入による共通化は JS 無効時の非表示・初期描画のチラつきという副作用があるため、現方針では採用しない。
